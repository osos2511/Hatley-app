...
class TrackOrderWidget extends StatefulWidget {
  final int? orderId;
  const TrackOrderWidget({super.key, this.orderId});
  @override
    ...
    return BlocBuilder<TrackingCubit, TrackingState>(
      builder: (context, state) {
        if (state is TrackingLoaded) {
          final TrakingResponse? relevantOrder = state.trackingData.firstWhereOrNull(
                (order) => order.orderId == widget.orderId,
          );
          if (relevantOrder == null) {
            return Center(child: Text("Order ID ${widget.orderId ?? 'undefined'} data not found."));
          }

          final int currentStepIndex = mapStatusToUiStepIndex(relevantOrder.status);
          final OrderStatus currentOrderStatusEnum = getOrderStatusEnum(relevantOrder.status);
              ...
              child: Column(
                ...
                children: [
                  ...
                  SizedBox(
                    height: 100,
                    child: Row(
                      children: List.generate(uiTrackingSteps.length, (stepIdx) {
                        final bool isActive = currentStepIndex >= 0 && stepIdx <= currentStepIndex;
                        final bool isLastStep = stepIdx == uiTrackingSteps.length - 1;
                                ...
                                children: [
                                  if (stepIdx != 0)
                                    Expanded(
                                      child: Container(
                                        height: 3,
                                        color: (stepIdx <= currentStepIndex) ? Colors.blue : Colors.grey,
                                      ),
                                    ),
                                  Container(
                                    width: 30,
                  ...
                  Center(
                    child: ElevatedButton(
                      onPressed: (relevantOrder.status == 3)
                          ? () => print('Rate button pressed for Order ID: ${relevantOrder.orderId}')
                          : null,
